...
## 강체 운동
### 개념
1. 평면에서 고정 좌표계 {s}에서 물체 좌표계 {b}를 표현 할 때, 얼마만큼 회전해야 하는지를 나타내는 회전 행렬(Rotation matrix)과 어느 방향으로 얼마만큼 이동 했는지를 나타내는 벡터가 필요하며 (<회전 행렬>, <벡터>)로 표현한다.
2. 평면에서 물체 좌표계 {b}의 각 축을 나타내는 단위 벡터를 나열하면 [ $\hat{x}$ , $\hat{y}$]가 되는데, 이 물체 좌표계의 축을 고정 좌표계의 축 기준으로 얼마나 회전했는지를 나타내는 θ로 표현하면  
```math
\begin{bmatrix}
\cos\theta & -\sin\theta \\
\sin\theta & \cos\theta
