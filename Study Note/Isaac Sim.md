...
- **Prim:** primitive의 줄임말로 장면을 구성하는 기본 요소(컴퓨터의 비트, 그래프의 노드 등으로 이해해 봄) + 여러 Prim을 담는 컨테이너의 역할을 할 수 도 있으며, 계층구조를 가질 수 있음 + Transform의 성질을 지님
...
- 시뮬레이션의 프레임을 업데이트하는 것 없이 로봇을 동작시키는 코드를 구현하더라도 제대로 작동하지 않음
